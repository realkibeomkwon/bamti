...
### 3-2. 누르기 전에 망을 네 갈래로 나눈다

**무엇을 물어야 하는지는 클릭하기 전에 이미 알 수 있습니다.** `WLAN_AVAILABLE_NETWORK`의
`dot11DefaultAuthAlgorithm`이 개인용과 기업용을 가릅니다. 프로필을 만들다 실패한 뒤에 물러서는
방식으로 짜지 마십시오. 그러면 사용자가 비밀번호를 넣고 나서야 실패를 봅니다.

이 컴퓨터의 SDK(`10.0.22621.0`의 `shared/wlantypes.h`)에 아래 값이 모두 있습니다. 실측했습니다.

| `dot11DefaultAuthAlgorithm` | 값 | 갈래 | 눌렀을 때 |
| --- | ---: | --- | --- |
| `DOT11_AUTH_ALGO_80211_OPEN` (암호화 없음) | 1 | 개방 | 바로 연결 |
| `DOT11_AUTH_ALGO_OWE` | 10 | 개방(암호화) | 바로 연결 |
| `DOT11_AUTH_ALGO_WPA_PSK` | 4 | 개인용 | 비밀번호만 받음 |
| `DOT11_AUTH_ALGO_RSNA_PSK` | 7 | 개인용 | 비밀번호만 받음 |
| `DOT11_AUTH_ALGO_WPA3_SAE` | 9 | 개인용 | 비밀번호만 받음 |
| `DOT11_AUTH_ALGO_80211_SHARED_KEY` | 2 | 개인용(WEP) | 비밀번호만 받음 |
| `DOT11_AUTH_ALGO_WPA` | 3 | 기업용 | 3-6절로 넘김 |
| `DOT11_AUTH_ALGO_RSNA` | 6 | 기업용 | 3-6절로 넘김 |
| `DOT11_AUTH_ALGO_WPA3` / `WPA3_ENT_192` | 8 | 기업용 | 3-6절로 넘김 |
| `DOT11_AUTH_ALGO_WPA3_ENT` | 11 | 기업용 | 3-6절로 넘김 |
| 그 밖 (`IHV` 범위 포함) | | 알 수 없음 | 3-6절로 넘김 |

`WifiNetwork`에 이 갈래를 담을 필드를 하나 두고 `RefreshPageLists`에서 채우십시오.
`bSecurityEnabled`만으로는 가를 수 없습니다.

`_PSK`와 `_SAE`로 끝나는 값이 개인용이고, 그것을 뗀 값(`WPA`, `RSNA`, `WPA3`)이 기업용입니다.
이름이 비슷해서 헷갈리기 쉬우니 표를 그대로 옮기십시오.
...
| `DOT11_AUTH_ALGO_80211_SHARED_KEY` | `open` + WEP |
...
`WPA3SAE`는 Windows 11에서만 받습니다. `WlanSetProfile`이 이 값을 거부하면 3-6절로 넘기고,
거부한 사실을 로그에 남기십시오.
...
### 3-6. 아이디까지 받아야 하는 망은 Windows에 넘긴다

기업용(802.1X) 망은 아이디와 비밀번호를 함께 받습니다. 학교나 회사의 망, `eduroam` 같은 것입니다.
**이 갈래는 우리가 처리하지 마십시오.** 입력란을 열지 말고 곧바로 `ms-availablenetworks:`를 열어
Windows의 네트워크 목록에 넘기십시오. 넘긴 사실과 인증 방식 값을 로그에 남깁니다.

칸 두 개를 더 그리면 될 것 같지만 그렇지 않습니다. 802.1X는 자격 증명 말고도 다음이 더 필요합니다.

1. **EAP 방식을 알아낼 방법이 없습니다.** PEAP인지 TTLS인지 TLS인지는 비컨에 실려 오지 않습니다.
   서버에 붙어 봐야 알 수 있고, 방식마다 프로필 XML의 `<EapHostConfig>` 블록이 다릅니다.
2. **서버 인증서를 검증할지 우리가 정하게 됩니다.** 검증을 끄면 붙기는 쉬워지지만 중간자 공격에
   그대로 노출됩니다. 사용자의 회사 계정이 걸린 문제를 우리가 대신 정할 자리가 아닙니다.
   켜려면 어느 루트 CA를 신뢰할지 알아야 하는데 그 정보는 망 쪽에서 따로 받습니다.
3. 익명 신원과 도메인 같은 항목이 더 필요한 경우가 흔합니다.

맥도 이 갈래는 별도의 시트를 띄우고 인증서를 신뢰할지 따로 묻습니다. 우리 쪽에서 Windows 창이
뜨는 것이 그 자리에 해당합니다.

프로필 생성이나 연결이 우리 예상과 다르게 실패했을 때도 같은 곳으로 넘기십시오.

### 3-7. 정리

| 상황 | 하는 일 |
| --- | --- |
| 이미 연결된 망 | 아무 것도 하지 않음 |
| 프로필이 있는 망 | `wlan_connection_mode_profile`로 바로 연결 |
| 개방 또는 OWE | `discovery_unsecure`로 바로 연결 |
| 개인용, 프로필 없음 | 입력란을 열고, 받은 비밀번호로 프로필을 만든 뒤 연결 |
| 기업용 또는 알 수 없음 | `ms-availablenetworks:`를 엶 |
...
6-1. **기업용 망.** 인증 방식이 3-2절의 기업용 갈래인 망을 눌렀을 때, 비밀번호 입력란이 뜨지 않고
   곧바로 Windows의 네트워크 목록이 열리는지 보십시오. 주변에 그런 망이 없으면 갈래를 판정하는
   자리에 임시로 값을 박아 한 번 확인하고, **확인한 뒤 그 임시 코드를 반드시 지우십시오.**
   주변 망들의 SSID와 `dot11DefaultAuthAlgorithm` 값을 한 번씩 로그로 남겨 어떤 갈래로 갈렸는지
   보고하십시오.
7. **특수 문자.** `&`가 든 비밀번호로 한 번 시도해 프로필이 깨지지 않는지 보십시오.
   실제 망이 없으면 만들어진 XML을 로그가 아닌 임시 파일로 한 번 떨어뜨려 확인하고,
   확인한 뒤 그 파일을 지우고 코드도 지우십시오.
8. **입력란이 떠도 팝업이 닫히지 않는지.** 입력란에 포커스가 갈 때 패널이 사라지면 3-3절의 예외
   처리가 빠진 것입니다.
...
- 3-6절로 넘긴 망이 있었다면 그 SSID와 인증 방식 값.
- 10번의 확인 결과.
