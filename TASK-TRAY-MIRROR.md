...
오버플로 안의 아이콘을 미러하지 못한다는 것은 실질적인 기능 저하입니다. 설정 화면과 문서에 "Windows 설정에서 모든 아이콘을 항상 표시하도록 바꾸면 전부 미러됩니다"라고 안내하십시오.
...
상단바 빈 곳 우클릭 메뉴에 "트레이 미러"와 "시스템 아이콘도 표시" 두 항목을 더합니다. 4단계에서 위젯 토글을 붙인 자리와 같은 방식입니다.
...
2. 오버플로에 숨겨진 아이콘은 미러되지 않으며, Windows 설정에서 모든 아이콘을 표시하도록 바꾸면 해결된다는 사실.
