...
시각: 2026-09-01 08:02. 장치 `스피커(Realtek(R) Audio)`. Release `build\Release\bamti.exe`.

```
2026-09-01 08:02:54.563 [widget] volume set 0.0000 -> read 0.0000
2026-09-01 08:02:54.993 [widget] volume set 1.0000 -> read 1.0000
```

왕복 오차 없음. 3-3은 적용하지 않음.

채움은 손잡이 중심(`cx`)까지 그린다. 손잡이는 흰색이고 채움은 강조색이라 색이 다르지만, 50%에서 채움 끝과 손잡이 중심이 같아야 하므로 채움을 `cx + thumb_r`까지 연장하지 않았다.
