...
시각: 2026-09-01. Release `build\Release\bamti.exe`. 논리 3413×1440, DPI 144, 상단바 물리 5120×48.

### 10-1 화면

- 시작 로고: 왼쪽 위는 `#4DD2FF`에 가까운 하늘색, 오른쪽 아래는 `#0078D4` 쪽 진한 파랑. 바깥 네 귀퉁이만 둥글고 가운데 십자는 각짐. 타일 간격은 이전 5%보다 얇음.
- 배터리: 가로 몸통·단자, 100%에서 채움이 가득. 전원 연결됨. 충전 중일 때 몸통 안에 번개가 보임. 상단바 퍼센트 글자는 테마 색.
- 배터리 패널: 잔량 막대가 아이콘과 같은 초록(`#0F7B0F`). 문구 `연결됨`.
- CPU: 고리가 12시부터 시계 방향. 23%는 초록, 34%는 `#9D5D00` 계열, 어두운 테마 56%는 노랑.
- 볼륨 62%: Segoe Fluent Icons 스피커(음파 있음). 음소거 패널 토글은 이번 클릭이 빗나갔고, 코드 경로는 `U+E74F`.
- 네트워크 상단바: `8.4KB/s · 3.2KB/s` 꼴. 패널: `누적 받기 1.3 MB · 보내기 638 KB`. `bamti 시작 이후` 없음.
- 밝은 테마와 어두운 테마(`SystemUsesLightTheme` 0/1) 모두 상단바가 따라옴. 확인 후 밝은 테마로 되돌림.

### 10-2 draw_ms

`NotePerf` 100프레임 평균. 위젯(배터리·CPU·네트워크·볼륨·트레이) 켠 상태.

수정 전:

```
2026-09-01 08:12:25.318 draw=0.04
2026-09-01 08:13:43.254 draw=0.04
```

평소 0.03–0.04ms. 가끔 0.23–0.46(콜드).

수정 후:

```
2026-09-01 08:44:14.658 draw=0.42
2026-09-01 08:45:14.365 draw=0.07
2026-09-01 08:46:45.489 draw=0.10
2026-09-01 08:47:57.866 draw=0.07
2026-09-01 08:49:07.359 draw=0.06
2026-09-01 08:50:18.585 draw=0.14
```

첫 표본 0.42는 콜드(수정 전 08:07:17도 0.46). 이후 정상 구간은 0.06–0.14ms. 증가량은 약 0.02–0.10ms로, 한도 0.5ms를 넘지 않음.

### 10-3 DPI

OS 배율은 바꾸지 않음. `EnsureLogo`는 `logo_geom_size_`가 달라지면 지오메트리와 브러시를 다시 만들고, `SetDpi`는 `DropTarget`으로 렌더 타깃 묶인 브러시를 버림.
