...
오버플로에 숨겨진 아이콘은 미러하지 않습니다. Windows 설정에서 아이콘을 항상 표시하면 미러됩니다. 우클릭 컨텍스트 메뉴는 앱에 직접 전달하지 못하며, 미러 항목의 우클릭 메뉴에서 알림 영역을 잠시 보여 그 메뉴에 도달합니다. 이 빌드에서는 아이콘 픽셀 캡처가 없어 툴팁 첫 글자 글리프로 표시합니다.
