...
오버플로에 숨긴 아이콘도 미러합니다. 설정에서 "숨긴 아이콘도 표시"를 끌 수 있습니다. 닫힌 상태 좌클릭 `Invoke`는 `HRESULT 0x00000000`이었고 `DoDefaultAction`은 호출하지 않았습니다. 전면 창이 바로 나타나지 않으면 우클릭 메뉴의 "알림 영역 잠시 표시"로 여십시오. 우클릭 컨텍스트 메뉴는 앱에 직접 전달하지 못하며, 미러 항목의 우클릭 메뉴에서 알림 영역을 잠시 보여 그 메뉴에 도달합니다. 이 빌드에서는 아이콘 픽셀 캡처가 없어 툴팁 첫 글자 글리프로 표시합니다.
