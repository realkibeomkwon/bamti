...
| `ping` | 하트비트. 상태를 바꾸지 않습니다. 클라이언트는 하트비트 주기(15초)의 절반 이하 간격으로 `ping`을 보내야 합니다. |
...
기본값은 전부 꺼짐입니다. 상단바 우클릭 메뉴에서 켭니다.
...
오버플로에 숨긴 아이콘도 미러합니다. 설정에서 "숨긴 아이콘도 표시"를 끌 수 있습니다. 기본 백엔드(UIA)는 아이콘 픽셀이 없어 툴팁 첫 글자 글리프로 표시하고, 우클릭은 앱에 전달하지 못합니다. 닫힌 상태 좌클릭 `Invoke`는 `HRESULT 0x00000000`이었고 `DoDefaultAction`은 호출하지 않았습니다. 전면 창이 바로 나타나지 않으면 우클릭 메뉴의 "알림 영역 잠시 표시"로 여십시오.

가로채기 백엔드(`tray_backend`: `"intercept"`)를 켜면 등록 메시지의 `HICON`을 PNG로 발행하고, 우클릭은 소유 앱의 컨텍스트 메뉴로 전달합니다. 상단바 우클릭 메뉴의 "트레이 아이콘 가로채기(실험)"으로 고릅니다. 스파이 창 생성 실패나 파싱 연속 실패 때는 UIA로 돌아갑니다.
...
벡터 아이콘은 내장 위젯 전용이며 파이프로 받지 않습니다. `StatusRow::fill_rgb`도 내장 전용입니다.
...
{"v":2,"op":"event","id":"...","event":"scroll","row_id":"volume_level","value":0.020}
```
...
`scroll`은 상단바 세그먼트 위에서 휠을 돌렸을 때 나갑니다. `value`는 상대 증감이며 단위 스케일(0.0~1.0 축)입니다. 한 눈금이 `+0.02` 또는 `-0.02`입니다. 받는 쪽은 현재값에 더한 뒤 0.0~1.0으로 잘라 씁니다. 처리하지 않는 항목은 조용히 무시합니다.
...
| 전체 항목 | 64 | 위와 같습니다. |
| 한 줄 | 64KB | 그 연결을 끊습니다. |
| 하트비트 | 15초 | 무응답이면 그 클라이언트의 항목을 제거합니다. 클라이언트는 7.5초 이하 간격으로 `ping`을 보내야 합니다. |
