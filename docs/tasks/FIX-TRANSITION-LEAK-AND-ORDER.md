...
| # | 확인 | 결과 |
| --- | --- | --- |
| 1 | 감출 때 중간 창이 드러나지 않음 (되돌아가지 않았는지) | 통과 (2026-09-06) |
| 2 | 되살릴 때 중간 창이 드러나지 않음 | **미해결. 아래 참고** |
| 3 | 열 번 왕복 뒤에도 순서가 어긋나지 않음 | 통과 |
| 4 | 감춘 상태에서 작업 표시줄로 창 하나를 직접 되살려도 이후 동작이 멀쩡함 | 통과 |

**애니메이션이 돌아오는지는 묻지 마십시오.** 2-1절대로 이 컴퓨터에서는 원래 없습니다.

3-3절의 측정값은 양쪽 모두 실패가 없었습니다.

```
22:50:52.205 [peek] transitions off n=7 fail=0
22:50:52.620 [peek] transitions on  n=7 fail=0
```

곧 `DwmSetWindowAttribute`는 끄는 방향과 켜는 방향 모두 다른 프로세스의 창에 정상으로 듣습니다. 3-3절 갈림길의 1번이고, 남은 2번의 원인은 **되살리기가 근본적으로 비동기라는 것 하나**로 좁혀졌습니다.

### 5-4. 2번을 여기서 닫습니다

**사용자가 2026-09-06에 현재 상태로 만족한다고 판단했습니다.** 중대한 문제가 아니라고 보았습니다.

다시 손댈 일이 생기면 출발점은 4절입니다. 창이 실제로 복원되기를 확인한 뒤 순서를 정하는 방식이 다음 후보이고, **UI 스레드를 붙잡을 위험이 있으므로 그 위험부터 따져야 합니다.** 남은 증상의 크기에 견주어 값이 맞는지 먼저 판단하십시오.
