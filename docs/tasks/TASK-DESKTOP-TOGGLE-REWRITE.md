...
| # | 확인 | 결과 |
| --- | --- | --- |
| 1 | Firefox를 맨 앞에 둔 채 Ctrl + 코너 → 바탕 화면이 드러나고 터미널이 올라오지 않음 | |
| 2 | 이어서 다시 Ctrl + 코너 → Firefox가 맨 앞으로 돌아옴 | |
| 3 | 1번과 2번을 연달아 다섯 번 반복 → 매번 방향이 맞음 | |
| 4 | 창을 드래그하는 도중에 Ctrl을 눌러 코너로 → 전환됨. 실패 시 그때의 `[peek]` 줄 | |
| 5 | 전환 도중에 중간 창이 드러나 보이지 않음 | |
