...
| 기존 트레이 아이콘 | explorer가 계속 소유한다. bamti는 읽기 전용으로 미러링하고 좌클릭을 전달한다 |

“기존 Windows 앱을 새 UI에서 사용”은 **창이 작업 영역 안에서 정상 동작하는 것**을 뜻한다. 트레이 본문이 상단바로 이사하는 것은 Status Item을 구현한 앱에만 해당한다.

### 9.1 트레이 미러

explorer의 알림 영역에 쓰지 않는다. UI Automation으로 버튼을 읽고 좌클릭은 `Invoke`로 위임한다. 우클릭 컨텍스트 메뉴를 앱에 직접 전달하지 못하며, 미러 항목 우클릭의 “알림 영역 잠시 표시”로 10초간 태스크바를 되돌린다. 오버플로에 숨긴 아이콘은 `TopLevelWindowForOverflowXamlIsland`의 자식 브리지에서 열거한다. 설정 `tray_overflow_icons`로 끌 수 있다. 닫힌 상태 `Invoke`는 성공(`0x00000000`)을 보고했으나 전면 창이 바로 보이지는 않았다. 이 컴퓨터의 `kParkedVisible` PrintWindow는 검정이므로 아이콘 그림 대신 툴팁 첫 글자를 그린다.
...
- **트레이 미러**는 explorer 알림 영역을 읽기만 하므로 가로채기보다 위험이 낮다. 이 컴퓨터에서는 아이콘 픽셀 캡처가 실패해 글리프 폴백이며, 우클릭은 앱에 직접 전달하지 못한다.
- **트레이 가로채기**(`Shell_TrayWnd`로 `WM_COPYDATA`를 대신 받기)는 기본값 꺼짐인 실험 기능이며 이번 범위에 넣지 않는다.
