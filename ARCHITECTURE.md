...
알림 영역 아이콘은 explorer가 계속 소유한다. bamti는 읽기 전용으로 미러하고 좌클릭만 UIA Invoke로 위임한다. 우클릭 메뉴와 오버플로 아이콘은 미러하지 않으며, 필요하면 알림 영역을 잠시 되돌린다.
...
explorer의 알림 영역에 쓰지 않는다. UI Automation으로 버튼을 읽고 좌클릭은 `Invoke`로 위임한다. 우클릭 컨텍스트 메뉴를 앱에 직접 전달하지 못하며, 미러 항목 우클릭의 “알림 영역 잠시 표시”로 10초간 태스크바를 되돌린다. 오버플로에 숨겨진 아이콘은 열거할 수 없다. Windows 설정에서 아이콘을 항상 표시하면 미러된다. 이 컴퓨터의 `kParkedVisible` PrintWindow는 검정이므로 아이콘 그림 대신 툴팁 첫 글자를 그린다.
