...
- 기존 `NOTIFYICON`을 맥 메뉴바 텍스트로 “자동 변환”하는 일
- 독 show/hide 애니메이션
...
1.0에서 시스템 트레이 아이콘을 상단바에 다시 그리는 것은 목표에 넣지 않는다. 알림 영역이 필요하면 상단바 버튼이 시스템의 알림 영역·퀵 설정을 연다.
...
트레이 `HICON` + 툴팁은 상시 본문을 표현하지 못한다. bamti가 보장하는 상시 텍스트는 이 프로토콜뿐이다.

### 8.1 전송

- IPC: named pipe, 사용자 세션 로컬
- 이름: `\\.\pipe\bamti-status`
- 메시지: UTF-8 JSON, 레코드 하나는 개행으로 구분하는 NDJSON
- 권한: 동일 사용자만. 관리자 앱이 일반 bamti에 붙는 시나리오는 후순위
...
```json
{"v":1,"op":"upsert","id":"vendor.app.usage","text":"Opus 42%","tooltip":"Anthropic usage","priority":10}
{"v":1,"op":"remove","id":"vendor.app.usage"}
```

- `id`: 앱이 고르는 안정적인 문자열. 같은 id는 덮어쓴다.
- `text`: 상단바에 그릴 짧은 문자열. 길이 상한을 둔다 (대략 32자, 넘치면 말줄임).
- `icon` (선택): 작은 PNG/ICO 경로 또는 공유 메모리. 1.0은 텍스트만으로도 된다.
- `priority`: 공간이 부족할 때 낮은 값부터 접는다.

bamti → 앱: 클릭·우클릭 이벤트. 앱이 파이프를 닫으면 해당 아이템을 제거한다. 비정상 종료에 대비해 짧은 하트비트를 둔다.

bamti는 이 파이프의 서버다. 사용량 표시 앱이 클라이언트로 붙는다. 1.0에서 기존 트레이 툴팁을 스크레이핑해 `text`를 채우지 않는다.
...
| Jump List | 후순위 |
| Windows 11 위젯 보드 | 임베드하지 않는다. 필요하면 위젯 앱을 실행하는 버튼만 |
| Rainmeter 등 데스크톱 위젯 | 작업 영역 변경만 영향을 준다. bamti가 호스팅하지 않는다 |
| 기존 트레이 아이콘 | 시스템 알림 영역이 담당. bamti가 재호스팅하지 않는다 |
...
7. 전체화면 시 즉시 숨김
8. 설정 창
9. (후순위) 트레이 호스팅, `ITaskbarList3` 배지, 독 작업 영역 예약 모드

## 13. 리스크

- **트레이 재호스팅**은 빌드 호환성과 정면으로 충돌한다. 코어에 넣지 않는다.
