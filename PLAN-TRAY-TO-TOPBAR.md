...
src/icon_cache.hpp / .cpp     HICON, PNG, 파일 경로를 ID2D1Bitmap으로 (dock.cpp에서 추출)
```
...
| 바 전체 다시 그리기 | 2ms 미만 | `Log(L"perf", ...)` |
| 세그먼트 하나만 다시 그리기 | 0.3ms 미만 | 위와 같음 |
...
| 3 | 아이콘 캐시 추출 | `refactor: 아이콘 비트맵 캐시를 공용 모듈로 옮긴다` |
| 4 | 프로토콜 v2 모델과 파서 | `feat: 상태 항목 프로토콜에 아이콘, 상태, 행 종류를 추가한다` |
...
표의 1번과 2번은 서로 독립적이므로 순서를 바꿔도 됩니다. 8번은 반드시 1번 이후에, 6번은 반드시 4번 이후에 진행합니다. 표의 번호는 커밋 단위이며, 4절의 단계 번호와는 다릅니다.
...
1. 이 컴퓨터(Windows 11 빌드 26200)에서 레거시 `ToolbarWindow32` 백엔드가 동작하는지 여부. 1단계 탐침이 답을 주며, 그 답이 5-0절 표를 통해 구현 범위를 바꿉니다. 비어 있을 것으로 예상하지만 측정 전에는 단정하지 않습니다.
2. UIA 백엔드로 아이콘 픽셀을 얻는 방법이 실용적인지 여부. `PrintWindow(PW_RENDERFULLCONTENT)`가 화면 밖 태스크바에서 정상 결과를 주는지 확인이 필요합니다. 실패하면 대안은 소유 프로세스의 실행 파일 아이콘을 대신 쓰는 것이며, 이는 충실도가 낮습니다.
